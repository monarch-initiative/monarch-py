...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            SolrQuery(rows=10),
            "q=%2A%3A%2A&rows=10&start=1&facet=false&facet_min_count=1",
        ),
        (
            SolrQuery(start=101),
            "q=%2A%3A%2A&rows=20&start=101&facet=false&facet_min_count=1",
        ),
        (
            SolrQuery(q="marfan"),
            "q=marfan&rows=20&start=1&facet=false&facet_min_count=1",
        ),
        (
